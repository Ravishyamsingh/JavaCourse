...
public class UserControllerIntegrationTest {
    ...
    `,
    exercise: `
1) Create a @RestController class with @GetMapping and @PostMapping annotations for handling HTTP requests.
2) Implement proper HTTP status codes (200, 201, 404, 400) in controller response methods.
3) Create a REST API endpoint that accepts JSON request body using @RequestBody annotation.
4) Implement @PathVariable to extract values from URL path parameters in a GET endpoint.
5) Add @RequestParam annotations to handle query parameters in a search endpoint.
`
  }
};