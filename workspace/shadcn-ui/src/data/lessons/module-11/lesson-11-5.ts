...
export const lesson_11_5: LessonContent = {
  ...
  content: {
    ...
    `,
    exercise: `
Question 1: Use next() method to iterate through a ResultSet and extract data using column names.
Question 2: Create a scrollable ResultSet and use absolute() and relative() methods for navigation.
Question 3: Extract ResultSet metadata and display column information for all columns.
Question 4: Create an updatable ResultSet and use updateRow() to modify existing data.
Question 5: Handle NULL values in ResultSet by using wasNull() method after data extraction.
`
  }
};