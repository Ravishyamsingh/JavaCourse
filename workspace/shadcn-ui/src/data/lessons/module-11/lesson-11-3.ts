...
export const lesson_11_3: LessonContent = {
  ...
  content: {
    ...
    `,
    exercise: `
Question 1: Create a Statement object from a Connection and use executeUpdate() to create a table.
Question 2: Use executeQuery() to select all records from a table and process the ResultSet.
Question 3: Implement batch processing by adding multiple INSERT statements and executing them together.
Question 4: Use execute() method to handle unknown SQL statement types and check if it returns a ResultSet.
Question 5: Handle SQL exceptions properly by catching SQLException and displaying error details.
`
  }
};