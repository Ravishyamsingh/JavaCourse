...
export const lesson_11_2: LessonContent = {
  ...
  content: {
    ...
    `,
    exercise: `
1. Establish a database connection using DriverManager.getConnection() with URL, username, and password.
2. Create a Properties object and use it to establish a database connection with custom parameters.
3. Use try-with-resources to properly manage database connection resources.
4. Validate a database connection using the isValid() method.
5. Extract and display database metadata from an established connection.
`
  }
};