...
export const lesson_13_4: LessonContent = {
  ...
  content: {
    ...
    `,
    exercise: `
Question 1: Implement an Observer pattern where a NewsAgency notifies multiple NewsChannel subscribers when news is published.
Question 2: Create a Strategy pattern for different sorting algorithms (BubbleSort, QuickSort) that can be selected at runtime.
Question 3: Implement a Command pattern for a remote control that can turn devices on/off with undo functionality.
Question 4: Explain the difference between Observer and Command patterns.
Question 5: When would you choose to use the Strategy pattern over inheritance?
    `
  }
};