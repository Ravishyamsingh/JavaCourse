...
export const lesson_10_7: LessonContent = {
  ...
  content: {
    ...
    `,
    exercise: `
1. Create an immutable Person class with final fields and provide a withAge() method for functional transformation.
2. Write a pure function that takes a list of integers and returns the sum using streams, ensuring no side effects.
3. Compose two functions using andThen() to first trim a string and then convert it to uppercase.
4. Implement error handling by wrapping a potentially throwing operation in Optional and using orElse() for fallback.
5. Create a higher-order function that takes a predicate and returns a function to filter and count elements in a list.
`
  }
};